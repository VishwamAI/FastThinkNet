...
def test_integration(model, data_pipeline):
    for images, labels in data_pipeline.take(1):
        ...
        images = images.view(images.size(0), model.input_channels, model.input_height, model.input_width).contiguous()  # Match model input shape
...
def test_error_handling(model):
    model.eval()  # Set the model to evaluation mode
    model.set_debug_mode(True)  # Enable debug mode
    with pytest.raises(InputShapeError):
        # Test with incorrect input shape (3D tensor)
        invalid_input = torch.randn(32, 28, 28)  # 3D tensor
        model(invalid_input)
