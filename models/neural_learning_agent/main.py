...
def train_model(model, env, episode_rewards, episodes=2000, gamma=0.99,
    ...
    for episode in tqdm(range(episodes)):
        ...
        while not done:
            ...
            if len(states) >= batch_size:
                update_model(model, target_model, np.array(states),
                             np.array(next_states), np.array(rewards),
                             np.array(actions), gamma)
                states, next_states, rewards, actions = [], [], [], []
...
def update_model(model, target_model, states, next_states, rewards, actions,
                 gamma):
    ...
    targets[np.arange(len(actions)), actions] = rewards + gamma * max_q_values
...
def test_model(model, env, episodes=100):
    ...
    print(f"Evaluation complete. Average reward over {episodes} episodes: "
          f"{avg_reward}")
...
def main():
    ...
    if args.test:
        ...
        epsilon_end = 0.01
        epsilon_decay = 0.995
        model, episode_rewards = train_model(
            model, env, episode_rewards, episodes=episodes,
